    ...
    $(function(){
        ...
        var renderEmployeeAccounts=function(listData){
            ...
            _.each(listData,function(employeeData){
                ...
                if(employeeData.isStop){    //已停用
                    ...
                    isStopHtmlStr='（<a href="javascript:;" class="reset-pwd-l fpwt-name-apv">重置密码</a>）';
                }
				var isAdminHtmlStr='';
                ...
                htmlStr+='<article class="employee-item fpw-text-wrap" enterpriseaccount="'+employeeData.enterpriseAccount+'">'+
                    '<h3 class="fpwt-name color-again">'+employeeData.enterpriseName+'</h3>'+
                    '<ul class="fpwt-list">'+
                        '<li><strong class="color-grey">企业帐号：</strong><span class="color-again">'+employeeData.enterpriseAccount+'</span></li>'+
                        '<li><strong class="color-grey">个人帐号：</strong><span class="color-again">'+employeeData.employeeAccount+isStopHtmlStr+'</span></li>'+
                        '<li><strong class="color-grey">个人姓名：</strong><span class="color-again">'+employeeData.employeeName+isAdminHtmlStr+'</span></li>'+
                    '</ul>'+
            ...
            if(listData.length>0){
                ...
                listEl.html('<p>此手机号或邮箱未绑定帐号，请确认后重试。</p><p class="prev-step-wrapper"><a href="#" class="prev-step-l">上一步</a></p>');
            }
        };
        /**
         * 通过enterpriseAccount获取企业账号列表信息
        ...
        (function(){
            ...
            sectionEl.on('click','.reset-pwd-l',function(evt){
                ...
                evt.preventDefault();
            }).on('touchstart','.employee-item',function(evt){
                var itemEl=$(this),
                    overlayEl=$('.overlay',itemEl);
                overlayEl.show();
            }).on('touchend','.employee-item',function(evt){
                /*var itemEl=$(this),
                    overlayEl=$('.overlay',itemEl);  */
                $('.overlay',sectionEl).hide();
                //overlayEl.hide();
            }).on('touchcancel','.employee-item',function(evt){
                /*var itemEl=$(this),
                    overlayEl=$('.overlay',itemEl);
                overlayEl.hide();*/
                $('.overlay',sectionEl).hide();
            }).on('touchmove','.employee-item',function(evt){	//touchend在某些系统中不会被触发，所以只好有移动就去掉选中效果
                //util.alert("test");
                $('.overlay',sectionEl).hide();
            });
        }());
                ...
                util.ajax({
                    ...
                    success: function(responseData){
                        ...
                        if (responseData.success) {
                            successTipEl.show();
                            //回填信息
                            util.externalExecute("SetLoginInfo",enterpriseData.enterpriseAccount+','+enterpriseData.employeeAccount+','+newPwd);
                            setTimeout(function(){
        ...
        //注册CloseWindow拦截函数
        //处于员工列表和重新密码view下需要提示是否关闭
        util.regInterceptor('IsCancelWindowClose',function(){
            //util.alert('android test');
            // util.alert('start');
            var employeeViewEl=$('#findpwd-choseuser-tpl'),
                resetViewEl=$('#findpwd-again-tpl');
            if(employeeViewEl.hasClass('tpl-state-active')||resetViewEl.hasClass('tpl-state-active')){
                if(util.confirm("找回密码尚未完成，是否退出？")){
                    util.externalExecute("CloseWindow");
                }
                return false;
            }else{
